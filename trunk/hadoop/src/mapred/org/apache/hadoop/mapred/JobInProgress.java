...
class JobInProgress {
  ...
  public JobInProgress(JobID jobid, JobTracker jobtracker, JobConf default_conf, int rCount)
    ...
    this.localJobFile = default_job_conf.getLocalPath(JobTracker.SUBDIR + "/" + jobid
        + ".xml");
    this.localJarFile = default_job_conf.getLocalPath(JobTracker.SUBDIR + "/" + jobid
        + ".jar");
    ...
    this.profile = new JobProfile(conf.getUser(), jobid, jobFile.toString(), url, conf
        .getJobName(), conf.getQueueName());
  ...
  public synchronized void initReduceTasks() {
    ...
    for (int i = 0; i < numReduceTasks; i++) {
      reduces[i] = new TaskInProgress(jobId, jobFile, numMapTasks, i, jobtracker, conf,
          this);
  ...
  public synchronized void initTasks() throws IOException, KillInterruptedException {
    if (tasksInited.get() || isComplete()) { return; }
    synchronized (jobInitKillStatus) {
      if (jobInitKillStatus.killed || jobInitKillStatus.initStarted) { return; }
      jobInitKillStatus.initStarted = true;
    }

//    LOG.info("Initializing " + jobId);
    LOG.info("@job "+System.currentTimeMillis()+" "+this.getJobID());
    ...
    for (int i = 0; i < numMapTasks; ++i) {
      ...
      maps.add(new TaskInProgress(jobId, jobFile, splits[i], jobtracker, conf, this, maps
          .size()));
    }
    ...
    cleanup[0] = new TaskInProgress(jobId, jobFile, emptySplit, jobtracker, conf, this,
        mapCleanupTaskID);
    ...
    // split.
    setup[0] = new TaskInProgress(jobId, jobFile, emptySplit, jobtracker, conf, this,
        mapSetupTaskID);
  ...
  public synchronized int pendingReduces() {
    return numReduceTasks - runningReduceTasks - failedReduceTIPs - finishedReduceTasks
  ...
  public synchronized void updateTaskStatus(TaskInProgress tip, TaskStatus status) {
    ...
    if (change) {
      ...
      if (state == TaskStatus.State.SUCCEEDED) {
        taskEvent = new TaskCompletionEvent(taskCompletionEventTracker, taskid, tip
            .idWithinJob(), status.getIsMap() && !tip.isJobCleanupTask()
            && !tip.isJobSetupTask(), TaskCompletionEvent.Status.SUCCEEDED,
            httpTaskLogLocation);
      ...
      // For a failed task update the JT datastructures.
        ...
        taskEvent = new TaskCompletionEvent(taskCompletionEventTracker, taskid, tip
            .idWithinJob(), status.getIsMap() && !tip.isJobCleanupTask()
            && !tip.isJobSetupTask(), taskCompletionStatus, httpTaskLogLocation);
      }
      ...
      if (taskEvent != null) {
        ...
        if ((finishedMapTasks + failedMapTIPs) == (numMapTasks)) {
          ...
          TaskCompletionEvent e = new TaskCompletionEvent(TaskCompletionEvent.Status.MAPDONE, numMapTasks);
          this.taskCompletionEvents.add(e);
          for (TaskCompletionEvent ee:taskCompletionEvents)
            LOG.info("e:"+ee.status+" "+ee.getTaskAttemptId());
        }
      }
    }
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug("Taking progress for " + tip.getTIPId() + " from " + oldProgress + " to "
          + tip.getProgress());
    }
  ...
  /**
   * Returns the total job counters, by adding together the job, the map and the
   * reduce counters.
   */
   ...
   */
  public synchronized Task obtainNewMapTask(TaskTrackerStatus tts, int clusterSize,
    ...
    int target = findNewMapTask(tts, clusterSize, numUniqueHosts, anyCacheLevel, status
        .mapProgress());
  ...
  public Task obtainTaskCleanupTask(TaskTrackerStatus tts, boolean isMapSlot)
      throws IOException {
    if (!tasksInited.get()) { return null; }
    synchronized (this) {
      if (this.status.getRunState() != JobStatus.RUNNING || jobFailed || jobKilled) { return null; }
      String taskTracker = tts.getTrackerName();
      if (!shouldRunOnTaskTracker(taskTracker)) { return null; }
      ...
      if (tip != null) { return tip.addRunningTask(taskid, taskTracker, true); }
      return null;
    }
  }

  public synchronized Task obtainNewLocalMapTask(TaskTrackerStatus tts, int clusterSize,
      int numUniqueHosts) throws IOException {
    if (!tasksInited.get()) {
      LOG.info("Cannot create task split for " + profile.getJobID());
      return null;
  ...
  public synchronized Task obtainNewNonLocalMapTask(TaskTrackerStatus tts, int clusterSize,
    ...
    int target = findNewMapTask(tts, clusterSize, numUniqueHosts, NON_LOCAL_CACHE_LEVEL,
        status.mapProgress());
    if (target == -1) { return null; }
  ...
  public Task obtainJobCleanupTask(TaskTrackerStatus tts, int clusterSize,
    ...
    synchronized (this) {
      if (!canLaunchJobCleanupTask()) { return null; }
   ...
   * 
   * Cleanup task can be launched if it is not already launched or job is Killed
  ...
  private synchronized boolean canLaunchJobCleanupTask() {
    ...
    if (status.getRunState() != JobStatus.RUNNING && status.getRunState() != JobStatus.PREP) { return false; }
    ...
    // zero.
    if (launchedCleanup || !isSetupFinished()) { return false; }
  ...
  public Task obtainJobSetupTask(TaskTrackerStatus tts, int clusterSize,
    ...
    synchronized (this) {
      if (!canLaunchSetupTask()) { return null; }
      String taskTracker = tts.getTrackerName();
      // Update the last-known clusterSize
      this.clusterSize = clusterSize;
      if (!shouldRunOnTaskTracker(taskTracker)) { return null; }
  ...
  public synchronized boolean scheduleReduces() {
    // LOG.info(getJobID() + " predecessors:" + predecessors.size() +
    // " finished map:"
    // + finishedMapTasks + " " + this.failedMapTIPs + " "
    // + numMapTasks);
    // LOG.info("schedule reduces: "+(predecessors.size() == 0 &&
    // finishedMapTasks == numMapTasks));
    if (conf.getBoolean("quickstart", false))
      return true;
    else
      return (predecessors.size() == 0 && (finishedMapTasks + failedMapTIPs) == (numMapTasks));
  }
  ...
  private synchronized boolean canLaunchSetupTask() {
    return (tasksInited.get() && status.getRunState() == JobStatus.PREP && !launchedSetup
        && !jobKilled && !jobFailed);
  }
  ...
  public synchronized Task obtainNewReduceTask(TaskTrackerStatus tts, int clusterSize,
    ...
    // scheduling reduces
    if (!scheduleReduces()) { return null; }

    int target = findNewReduceTask(tts, clusterSize, numUniqueHosts, status
        .reduceProgress());
    if (target == -1) { return null; }
   ...
   *          task-tracker status
   * @param isScheduled
   *          Whether this task is scheduled from the JT or has joined back upon
   *          restart
   */
  synchronized void addRunningTaskToTIP(TaskInProgress tip, TaskAttemptID id,
      TaskTrackerStatus tts, boolean isScheduled) {
//  LOG.info("@start "+System.currentTimeMillis()+" "+tip.getIdWithinJob()+"_"+id.getId());
    LOG.info("@start "+id);
...
//        LOG.info("Choosing rack-local task " + tip.getTIPId());
  ...
  static String convertTrackerNameToHostName(String trackerName) {
    ...
    String trackerHostName = (indexOfColon == -1) ? trackerName : trackerName.substring(0,
        indexOfColon);
  ...
  /**
   * Remove a reduce TIP from the list for running-reduces Called when a reduce
   * fails/completes
   * 
   * @param tip
   *          the tip that needs to be retired
   ...
   */
  private synchronized TaskInProgress findTaskFromList(Collection<TaskInProgress> tips,
   ...
   */
  private synchronized TaskInProgress findSpeculativeTask(Collection<TaskInProgress> list,
      TaskTrackerStatus ttStatus, double avgProgress, long currentTime, boolean shouldRemove) {

    int slots = tracker.totalMapTaskCapacity - runningMapTasks;
    // LOG.info("slots="+slots+" sd="+conf.getBoolean("speculative.dynamic",
    // false));

    if (!scheduleReduces()) {
      Iterator<TaskInProgress> iter = list.iterator();
      long max = 0;
      TaskInProgress mtip = null;
      while (iter.hasNext()) {
        TaskInProgress tip = iter.next();
        if (tip.isMapTask() && !subTaskMap.containsKey(tip) && !subTips.contains(tip)
            && !tip.hadSpeculativeTask) {
          long es = tip.estimate(currentTime);
          if (es > max) {
            max = es;
            mtip = tip;
          }
      ...
      if (mtip != null) {
        if (conf.getBoolean("dynamic.speculative", false)) {
          if (slots >= conf.getInt("dynamic.parts", 5)) {
            LOG.info("find dynamic speculative task:"+mtip.getTIPId());
            mtip.estimate(currentTime);
            createDynamicTasks(mtip, conf.getInt("dynamic.parts", 5));
            mtip.hadSpeculativeTask = true;
          }
          return null;
        } else {
          LOG.info("find speculative task:"+mtip.getTIPId());
          mtip.hadSpeculativeTask = true;
          return mtip;
        }
      }
    ...
    while (iter.hasNext()) {
      ...
      // if (tip.isMapTask() && conf.getBoolean("speculative.dynamic", false)) {
      // if (!subTaskMap.containsKey(tip) && !subTips.contains(tip)
      // && !tip.hadSpeculativeTask && tip.hasSpeculativeTask(currentTime,
      // avgProgress)) {
      // if (shouldRemove) {
      // iter.remove(); // this tracker is never going to run it again
      // }
      // createDynamicTasks(tip, conf.getInt("speculative.parts", 3));
      // return null;
      // }
      // }
   ...
   * @param maxCacheLevel
   *          The maximum topology level until which to schedule maps. A value
   *          of {@link #anyCacheLevel} implies any available task (node-local,
   *          rack-local, off-switch and speculative tasks). A value of
   *          {@link #NON_LOCAL_CACHE_LEVEL} implies only off-switch/speculative
  ...
  private synchronized int findNewMapTask(final TaskTrackerStatus tts,
    ...
    if (!shouldRunOnTaskTracker(taskTracker)) { return -1; }
    ...
    if (availSpace < outSize) {
      LOG.warn("No room for map task. Node " + tts.getHost() + " has " + availSpace
    ...
    if (node != null) {
      ...
      // speculative)
      // tasks or it might be NON_LOCAL_CACHE_LEVEL (i.e. -1) if findNewMapTask
      // is
    ...
    Node nodeParentAtMaxLevel = (node == null) ? null : JobTracker.getParentNode(node,
        maxLevel - 1);
...
//          LOG.info("Choosing a non-local task " + tip.getTIPId());
    ...
    if (hasSpeculativeMaps) {
      ...
      if (node != null) {
        ...
        for (int level = 0; level < maxLevel; ++level) {
          ...
          if (cacheForLevel != null) {
            tip = findSpeculativeTask(cacheForLevel, tts, avgProgress, currentTime,
                level == 0);
            if (tip != null) {
              if (cacheForLevel.size() == 0) {
   ...
   * @return the index in tasks of the selected task (or -1 for no task)
   */
  private synchronized int findNewReduceTask(TaskTrackerStatus tts, int clusterSize,
      int numUniqueHosts, double avgProgress) {
    if (numReduceTasks == 0) {
    ...
    long availSpace = tts.getResourceStatus().getAvailableSpace();
    if (availSpace < outSize) {
      LOG.warn("No room for reduce task. Node " + taskTracker + " has " + availSpace
    ...
    if (hasSpeculativeReduces) {
      tip = findSpeculativeTask(runningReduces, tts, avgProgress, System
  ...
  public void checkSubTasks(TaskInProgress tip) {
    if (subTaskMap.containsKey(tip)) {
      ...
      for (TaskInProgress stip : stips) {
        ...
        if (stip.isComplete()) {
          LOG.info(stip.getTIPId() + " has completed, just set status");
      ...
      if (flag) {
        LOG.info("all subtasks of " + ptip.getTIPId() + " completed, kill this tip");
        ptip.killAllTasks();
        for (TaskInProgress stip : subTaskMap.get(ptip)) {
          parentTaskMap.remove(stip);
        }
  ...
  private void jobComplete() {
    ...
    if (this.status.getRunState() == JobStatus.RUNNING) {
      ...
      // Note that finalize will close the job history handles which garbage
      // collect
  ...
  private synchronized void terminateJob(int jobTerminationState) {
    if ((status.getRunState() == JobStatus.RUNNING)
      ...
      if (jobTerminationState == JobStatus.FAILED) {
        this.status = new JobStatus(status.getJobID(), 1.0f, 1.0f, 1.0f, JobStatus.FAILED,
            status.getJobPriority());
        this.finishTime = System.currentTimeMillis();
      } else {
        this.status = new JobStatus(status.getJobID(), 1.0f, 1.0f, 1.0f, JobStatus.KILLED,
            status.getJobPriority());
        this.finishTime = System.currentTimeMillis();
      }
      garbageCollect();
   ...
   * marking the job as failed/killed. Cleanup tip will be launched. If the job
   * has not inited, it will directly call terminateJob as there is no need to
   * launch cleanup tip. This method is reentrant.
   * 
...
//    LOG.info("failed task " + tip.getTIPId() + ":" + taskid);
  ...
  public void failedTask(TaskInProgress tip, TaskAttemptID taskid, String reason,
      TaskStatus.Phase phase, TaskStatus.State state, String trackerName) {
    TaskStatus status = TaskStatus.createTaskStatus(tip.isMapTask(), taskid, 0.0f, state,
        reason, reason, trackerName, phase, new Counters());
    // update the actual start-time of the attempt
    TaskStatus oldStatus = tip.getTaskStatus(taskid);
    long startTime = oldStatus == null ? System.currentTimeMillis() : oldStatus
        .getStartTime();
    status.setStartTime(startTime);
  ...
  public synchronized TaskInProgress getTaskInProgress(TaskID tipid) {
    if (tipid.isMap()) {
      ...
      for (int i = 0; i < reduces.length; i++) {
        if (tipid.equals(reduces[i].getTIPId())) { return reduces[i]; }
      }
    }
    return null;
  }
  ...
  public synchronized TaskStatus findFinishedMap(int mapId) {
    ...
    if (tip.isComplete()) {
      ...
      for (int i = 0; i < statuses.length; i++) {
        if (statuses[i].getRunState() == TaskStatus.State.SUCCEEDED) { return statuses[i]; }
      }
    }
    return null;
  }
  ...
  synchronized public TaskCompletionEvent[] getTaskCompletionEvents(int fromEventId,
      int maxEvents) {
    ...
    for (TaskCompletionEvent e : events) {
      if (abandonedTips.contains(e.idWithinJob()) && e.status != TaskCompletionEvent.Status.MAPDONE) {
        // LOG.info("set event type for " + e.getTaskAttemptId() +
        // ": TipFailed<-" + e.status);
        e.status = TaskCompletionEvent.Status.TIPFAILED;
      }
      LOG.info("get task completion event:"+e.getTaskAttemptId()+" "+e.status);
    }
    return events;
  }

  synchronized void fetchFailureNotification(TaskInProgress tip, TaskAttemptID mapTaskId,
      String trackerName) {
  ...
  /**
   * To keep track of kill and initTasks status of this job. initTasks() take a
   * lock on JobInProgress object. kill should avoid waiting on JobInProgress
   * lock since it may take a while to do initTasks().
   */
  ...
  public boolean createDynamicTasks(int numParts) {
    TaskInProgress mtip = null;
    long max = 0;
    for (TaskInProgress tip : maps) {
      // LOG.info("try " + tip.getTIPId());
      // LOG.info(tip.isRunnable() + " " + tip.isRunning() + " " +
      // subTaskMap.containsKey(tip)
      // + " " + subTips.contains(tip));
      if (tip.isRunnable() && !tip.isRunning() && !subTaskMap.containsKey(tip)
          && !subTips.contains(tip)) {
        if (tip.getMapInputSize()>max) {
          max = tip.getMapInputSize();
          mtip = tip;
        }
      }
    }
    if (mtip != null) {
      createDynamicTasks(mtip, numParts);
      return true;
    }
    return false;
  }
  ...
  public void createDynamicTasks(TaskInProgress tip, int numParts) {
    // LOG.info("create dynamic sub tasks tip=" + tip.getTIPId() + " parts=" +
    // numParts);
    ...
    try {
      ...
      for (RawSplit split : splits) {
        ...
        subTips.add(stip);
        stips.add(stip);
        sids.add(stip.getIdWithinJob());
      }
      LOG.info("@dynamic " + tip.getIdWithinJob() + "->" + sids.toString());
      if (tip.isRunning()) {
        ...
        LOG.info("@annul "+System.currentTimeMillis()+" "+tip.getIdWithinJob());
      }
    } catch (IOException e) {
  ...
  void prepareMap() {
    int slots = tracker.totalMapTaskCapacity - runningMapTasks;
    int left = numMapTasks - finishedMapTasks - failedMapTIPs - runningMapTasks;
    int threshold = this.conf.getInt("dynamic.threshold", 0);
    int nparts = this.conf.getInt("dynamic.parts", 5);
    if (threshold == 0)
      return;
    if (predecessors.size() > 0)
      return;
    if (left == 0)
      return;
    if (left <= threshold && !replaced) {
      while(createDynamicTasks(nparts));
      LOG.info("created replace tasks");
      replaced = true;
    }
    if (slots >= nparts + left - 1) {
      if (createDynamicTasks(nparts)) {
        LOG.info("nmap:" + numMapTasks + " finished:" + finishedMapTasks + " failed:"
            + failedMapTIPs + " running:" + runningMapTasks + " empty:" + slots + " left:"
            + left);
      }
    }
  }
