...
class ReduceTask extends Task {
  ...
  private Counters.Counter reduceInputValueCounter = getCounters().findCounter(
  ...
  private Counters.Counter reduceCombineOutputCounter = getCounters().findCounter(
  ...
  private class ReduceValuesIterator<KEY, VALUE> extends ValuesIterator<KEY, VALUE> {
    public ReduceValuesIterator(RawKeyValueIterator in, RawComparator<KEY> comparator,
        Class<KEY> keyClass, Class<VALUE> valClass, Configuration conf,
  ...
  private class SkippingReduceValuesIterator<KEY, VALUE> extends
    ...
    public SkippingReduceValuesIterator(RawKeyValueIterator in,
        RawComparator<KEY> comparator, Class<KEY> keyClass, Class<VALUE> valClass,
        Configuration conf, TaskReporter reporter, TaskUmbilicalProtocol umbilical)
        throws IOException {
      super(in, comparator, keyClass, valClass, conf, reporter);
      this.umbilical = umbilical;
      this.skipGroupCounter = reporter.getCounter(Counter.REDUCE_SKIPPED_GROUPS);
      this.skipRecCounter = reporter.getCounter(Counter.REDUCE_SKIPPED_RECORDS);
    ...
    private void writeSkippedRec(KEY key, VALUE value) throws IOException {
      if (skipWriter == null) {
        ...
        skipWriter = SequenceFile.createWriter(skipFile.getFileSystem(conf), conf,
            skipFile, keyClass, valClass, CompressionType.BLOCK, reporter);
      }
  ...
  public void run(JobConf job, final TaskUmbilicalProtocol umbilical) throws IOException,
    ...
    if (!isLocal) {
      ...
      if (!reduceCopier.fetchOutputs()) {
        if (reduceCopier.mergeThrowable instanceof FSError) { throw (FSError) reduceCopier.mergeThrowable; }
        throw new IOException("Task: " + getTaskID() + " - The reduce copier failed",
    ...
    RawKeyValueIterator rIter = isLocal ? Merger.merge(job, rfs,
        job.getMapOutputKeyClass(), job.getMapOutputValueClass(), codec, getMapFiles(rfs,
            true), !conf.getKeepFailedTaskFiles(), job.getInt("io.sort.factor", 100),
        new Path(getTaskID().toString()), job.getOutputKeyComparator(), reporter,
        spilledRecordsCounter, null) : reduceCopier.createKVIterator(job, rfs, reporter);
  ...
  private <INKEY, INVALUE, OUTKEY, OUTVALUE> void runOldReducer(JobConf job,
      ...
      RawKeyValueIterator rIter, RawComparator<INKEY> comparator, Class<INKEY> keyClass,
    ...
    final RecordWriter<OUTKEY, OUTVALUE> out = job.getOutputFormat().getRecordWriter(fs,
    ...
    try {
      ...
      ReduceValuesIterator<INKEY, INVALUE> values = isSkipping() ? new SkippingReduceValuesIterator<INKEY, INVALUE>(
          rIter, comparator, keyClass, valueClass, job, reporter, umbilical)
          : new ReduceValuesIterator<INKEY, INVALUE>(rIter, job
              .getOutputValueGroupingComparator(), keyClass, valueClass, job, reporter);
  ...
  private <INKEY, INVALUE, OUTKEY, OUTVALUE> void runNewReducer(JobConf job,
    ...
    org.apache.hadoop.mapreduce.Reducer.Context reducerContext = createReduceContext(
        reducer, job, getTaskID(), rIter, reduceInputValueCounter, output, committer,
  ...
  class ReduceCopier<K, V> implements MRConstants {
    ...
    /**
     * the amount of time spent on fetching one map output before considering it
     * as failed and notifying the jobtracker about it.
     */
     ...
     * the specific metrics for shuffle. This class actually reports the metrics
     * for the shuffle client (the ReduceTask), and hence the name
    ...
    private class MapOutput {
      ...
      public MapOutput(TaskID mapId, TaskAttemptID mapAttemptId, Configuration conf,
          Path file, long size) {
        this.mapId = mapId;
    ...
    class ShuffleRamManager implements RamManager {
      ...
      public ShuffleRamManager(Configuration conf) throws IOException {
        ...
        if (maxInMemCopyUse > 1.0 || maxInMemCopyUse < 0.0) { throw new IOException(
            "mapred.job.shuffle.input.buffer.percent" + maxInMemCopyUse); }
        maxSize = (int) Math.min(Runtime.getRuntime().maxMemory() * maxInMemCopyUse,
            Integer.MAX_VALUE);
        maxSingleShuffleLimit = (int) (maxSize * MAX_SINGLE_SHUFFLE_SEGMENT_FRACTION);
        LOG.info("ShuffleRamManager: MemoryLimit=" + maxSize + ", MaxSingleShuffleLimit="
      ...
      public boolean waitForDataToMerge() throws InterruptedException {
        ...
        synchronized (dataAvailable) {
          ...
          // In-memory threshold exceeded and at least two segments
              ...
              (numPendingRequests < numCopiers * MAX_STALLED_SHUFFLE_THREADS_FRACTION && (0 == numRequiredMapOutputs || numPendingRequests < numRequiredMapOutputs))) {
            dataAvailable.wait();
          }
          done = closed;
        }
    ...
    private class MapOutputCopier extends Thread {
        ...
        while (true) {
          try {
            ...
            try {
              ...
              LOG.warn(reduceTask.getTaskID() + " copy failed: " + loc.getTaskAttemptId()
                  + " from " + loc.getHost());
      ...
      private long copyOutput(MapOutputLocation loc) throws IOException,
        ...
        if (copiedMapOutputs.contains(loc.getTaskId())
            || obsoleteMapIds.contains(loc.getTaskAttemptId())) { return CopyResult.OBSOLETE; }
        ...
        Path filename = new Path("/"
            + TaskTracker.getIntermediateOutputDir(reduceId.getJobID().toString(), reduceId
                .toString()) + "/map_" + loc.getTaskId().getId() + ".out");
        ...
        if (mapOutput == null) { throw new IOException("Failed to fetch map-output for "
        ...
        synchronized (ReduceTask.this) {
          ...
          if (mapOutput.inMemory) {
            ...
            if (!localFileSys.rename(tmpMapOutput, filename)) {
              ...
              bytes = -1;
              throw new IOException("Failed to rename map output " + tmpMapOutput + " to "
                  + filename);
            }
       ...
       */
      private MapOutput getMapOutput(MapOutputLocation mapOutputLoc, Path filename,
          int reduce) throws IOException, InterruptedException {
        // Connect
        ...
        if (!mapId.equals(expectedMapId)) {
          LOG.warn("data from wrong map:" + mapId + " arrived to reduce task " + reduce
        ...
        long compressedLength = Long
            .parseLong(connection.getHeaderField(MAP_OUTPUT_LENGTH));

        if (compressedLength < 0 || decompressedLength < 0) {
          LOG.warn(getName() + " invalid lengths in map output header: id: " + mapId
              + " compressed len: " + compressedLength + ", decompressed len: "
              + decompressedLength);
          return null;
        }
        ...
        if (forReduce != reduce) {
          LOG.warn("data for the wrong reduce: " + forReduce + " with compressed len: "
        ...
        if (shuffleInMemory) {
          LOG.info("Shuffling " + decompressedLength + " bytes (" + compressedLength
              + " raw bytes) " + "into RAM from " + mapOutputLoc.getTaskAttemptId());

          mapOutput = shuffleInMemory(mapOutputLoc, connection, input,
              (int) decompressedLength, (int) compressedLength);
        } else {
          LOG.info("Shuffling " + decompressedLength + " bytes (" + compressedLength
              + " raw bytes) " + "into Local-FS from " + mapOutputLoc.getTaskAttemptId());
       ...
       */
      private InputStream getInputStream(URLConnection connection, int connectionTimeout,
          int readTimeout) throws IOException {
        int unit = 0;
        if (connectionTimeout < 0) {
          throw new IOException("Invalid timeout " + "[timeout = " + connectionTimeout
              + " ms]");
        ...
        while (true) {
          try {
            ...
            if (connectionTimeout == 0) { throw ioe; }
      ...
      private MapOutput shuffleInMemory(MapOutputLocation mapOutputLoc,
        ...
        if (!createdNow) {
          ...
          try {
            ...
            input = getInputStream(connection, STALLED_COPY_TIMEOUT, DEFAULT_READ_TIMEOUT);
          } catch (IOException ioe) {
        ...
        MapOutput mapOutput = new MapOutput(mapOutputLoc.getTaskId(), mapOutputLoc
            .getTaskAttemptId(), shuffleData, compressedLength);
        ...
        try {
          ...
          try {
            ...
            LOG.info(
                "Failed to discard map-output from " + mapOutputLoc.getTaskAttemptId(),
                ignored);
          }
          mapOutput = null;
        ...
        if (bytesRead != mapOutputLength) {
          ...
          throw new IOException("Incomplete map output received for "
              ...
              + mapOutputLoc.getOutputLocation() + " (" + bytesRead + " instead of "
              + mapOutputLength + ")");
        }
        ...
        if (mapOutputLength > 0) {
          ...
          LOG.info("Rec #1 from " + mapOutputLoc.getTaskAttemptId() + " -> ("
              + WritableUtils.readVInt(dib) + ", " + WritableUtils.readVInt(dib)
              + ") from " + mapOutputLoc.getHost());
        }

        return mapOutput;
      }

      private MapOutput shuffleToDisk(MapOutputLocation mapOutputLoc, InputStream input,
          Path filename, long mapOutputLength) throws IOException {
        // Find out a suitable location for the output on local-filesystem
        Path localFilename = lDirAlloc.getLocalPathForWrite(filename.toUri().getPath(),
            mapOutputLength, conf);

        MapOutput mapOutput = new MapOutput(mapOutputLoc.getTaskId(), mapOutputLoc
            .getTaskAttemptId(), conf, localFileSys.makeQualified(localFilename),
            mapOutputLength);
        ...
        if (bytesRead != mapOutputLength) {
          try {
            ...
            // IGNORED because we are cleaning up
            LOG
                .info("Failed to discard map-output from "
    ...
    public ReduceCopier(TaskUmbilicalProtocol umbilical, JobConf conf, TaskReporter reporter)
      ...
      this.combinerRunner = CombinerRunner.create(conf, getTaskID(), combineInputCounter,
          reporter, null);
      if (combinerRunner != null) {
        combineCollector = new CombineOutputCollector(reduceCombineOutputCounter);
      }
      ...
      this.maxFailedUniqueFetches = Math.min(numMaps, this.maxFailedUniqueFetches);
      ...
      final float maxRedPer = conf.getFloat("mapred.job.reduce.input.buffer.percent", 0f);
      if (maxRedPer > 1.0 || maxRedPer < 0.0) { throw new IOException(
          "mapred.job.reduce.input.buffer.percent" + maxRedPer); }
      this.maxInMemReduce = (int) Math.min(Runtime.getRuntime().maxMemory() * maxRedPer,
          Integer.MAX_VALUE);
...
//      }
      ...
      while (mergeThrowable == null) {
        ...
        if (logNow) {
          LOG.info(reduceTask.getTaskID() + " Need another "
              + (numMaps - copiedMapOutputs.size()) + " map output(s) " + "where "
        ...
        if (numScheduled > 0 || logNow) {
          LOG.info(reduceTask.getTaskID() + " Scheduled " + numScheduled + " outputs ("
              + penaltyBox.size() + " slow hosts and" + numDups + " dup hosts)");
        }
        ...
        while (numInFlight > 0 && mergeThrowable == null) {
          ...
          if (cr.getSuccess()) { // a successful copy
            ...
            LOG.info(reduceTask.getTaskID()
                ...
                + cr.getLocation().getTaskAttemptId() + " from host: " + cr.getHost());
          } else {
            ...
            LOG.info("Task " + getTaskID() + ": Failed fetch #" + noFailedFetches
                + " from " + mapTaskId);
            ...
            if (noFailedFetches == maxFetchRetriesPerMap) {
              ...
              if ((fetchFailedMaps.size() >= maxFailedUniqueFetches || fetchFailedMaps
                ...
                LOG.fatal("Shuffle failed with too many fetch failures "
                    + "and insufficient progress!" + "Killing task " + getTaskID() + ".");
                umbilical.shuffleError(getTaskID(), "Exceeded MAX_FAILED_UNIQUE_FETCHES;"
                    + " bailing-out.");
              }
            }
            ...
            int currentBackOff = noFailedFetches <= maxFetchRetriesPerMap ? BACKOFF_INIT
                * (1 << (noFailedFetches - 1)) : (this.maxBackoff * 1000 / 2);
      ...
      if (mergeThrowable == null) {
        try {
          ...
          LOG.info("Interleaved on-disk merge complete: " + mapOutputFilesOnDisk.size()
              + " files left.");

          // wait for an ongoing merge (if it is in flight) to complete
          inMemFSMergeThread.join();
          LOG.info("In-memory merge complete: " + mapOutputsFilesInMemory.size()
              + " files left.");
        } catch (InterruptedException ie) {
    ...
    private long createInMemorySegments(List<Segment<K, V>> inMemorySegments,
      ...
      synchronized (mapOutputsFilesInMemory) {
        ...
        while (fullSize > leaveBytes) {
          ...
          fullSize -= mo.data.length;
          Reader<K, V> reader = new InMemoryReader<K, V>(ramManager, mo.mapAttemptId,
    ...
    private RawKeyValueIterator createKVIterator(JobConf job, FileSystem fs,
      ...
      if (mapOutputsFilesInMemory.size() > 0) {
        ...
        if (numMemDiskSegments > 0 && ioSortFactor > mapOutputFilesOnDisk.size()) {
          ...
          final Path outputPath = mapOutputFile.getInputFileForWrite(mapId, reduceTask
              .getTaskID(), inMemToDiskBytes);
          final RawKeyValueIterator rIter = Merger.merge(job, fs, keyClass, valueClass,
              memDiskSegments, numMemDiskSegments, tmpDir, comparator, reporter,
              spilledRecordsCounter, null);
          final Writer writer = new Writer(job, fs, outputPath, keyClass, valueClass,
              codec, null);
          try {
      ...
      if (0 != onDiskBytes) {
        ...
        RawKeyValueIterator diskMerge = Merger.merge(job, fs, keyClass, valueClass, codec,
            diskSegments, ioSortFactor, numInMemSegments, tmpDir, comparator, reporter,
      ...
      return Merger.merge(job, fs, keyClass, valueClass, finalSegments, finalSegments
          .size(), tmpDir, comparator, reporter, spilledRecordsCounter, null);
    }
      ...
      public void run() {
        try {
          ...
          while (!exitLocalFSMerge) {
            ...
            LOG.info(reduceTask.getTaskID() + "We have  " + mapOutputFilesOnDisk.size()
                + " map outputs on disk. " + "Triggering merge of " + ioSortFactor
                + " files");
            ...
            // sanity check
            if (mapFiles.size() == 0) { return; }

            // add the checksum length
            approxOutputSize += ChecksumFileSystem.getChecksumLength(approxOutputSize,
                bytesPerSum);

            // 2. Start the on-disk merge process
            Path outputPath = lDirAlloc.getLocalPathForWrite(mapFiles.get(0).toString(),
                approxOutputSize, conf).suffix(".merged");
            Writer writer = new Writer(conf, rfs, outputPath, conf.getMapOutputKeyClass(),
                conf.getMapOutputValueClass(), codec, null);
            RawKeyValueIterator iter = null;
            Path tmpDir = new Path(reduceTask.getTaskID().toString());
            try {
              iter = Merger.merge(conf, rfs, conf.getMapOutputKeyClass(), conf
                  .getMapOutputValueClass(), codec, mapFiles.toArray(new Path[mapFiles
                  .size()]), true, ioSortFactor, tmpDir, conf.getOutputKeyComparator(),
            ...
            LOG.info(reduceTask.getTaskID() + " Finished merging " + mapFiles.size()
    ...
    private class InMemFSMergeThread extends Thread {
      ...
      private void doInMemMerge() throws IOException {
        if (mapOutputsFilesInMemory.size() == 0) { return; }
        ...
        Path outputPath = mapOutputFile.getInputFileForWrite(mapId, reduceTask.getTaskID(),
            mergeOutputSize);
        ...
        try {
          LOG
              .info("Initiating in-memory merge with " + noInMemorySegments
                  + " segments...");

          rIter = Merger.merge(conf, rfs, (Class<K>) conf.getMapOutputKeyClass(),
              (Class<V>) conf.getMapOutputValueClass(), inMemorySegments, inMemorySegments
                  .size(), new Path(reduceTask.getTaskID().toString()), conf
                  .getOutputKeyComparator(), reporter, spilledRecordsCounter, null);
          ...
          LOG.info(reduceTask.getTaskID() + " Merge of the " + noInMemorySegments
    ...
    private class GetMapEventsThread extends Thread {
      ...
      public void run() {
        ...
        do {
          try {
            ...
            if (numNewMaps > 0) {
              LOG.info(reduceTask.getTaskID() + ": " + "Got " + numNewMaps
                  + " new map-outputs");
            }
            Thread.sleep(SLEEP_TIME);
          } catch (InterruptedException e) {
            LOG.warn(reduceTask.getTaskID() + " GetMapEventsThread returning after an "
      ...
      private int getMapCompletionEvents() throws IOException {
        ...
        MapTaskCompletionEventsUpdate update = umbilical.getMapCompletionEvents(reduceTask
            .getJobID(), fromEventId.get(), MAX_EVENTS_TO_FETCH, reduceTask.getTaskID());
        ...
        for (TaskCompletionEvent event : events) {
          ...
          case SUCCEEDED: {
            ...
            URL mapOutputLocation = new URL(event.getTaskTrackerHttp() + "/mapOutput?job="
                + taskId.getJobID() + "&map=" + taskId + "&reduce=" + getPartition());
   ...
   * value, or zero if value leq 0. This follows the observation that the msb of
   * a given value is also the closest power of two, unless the bit following it
   * is set.
   */
