...
int ChunkManager::init()
{
    ...
    LOG_INFO("Disk Avail: "<< m_disk_manager.getFreespaceM()<<" MB");
    return 1;
}
...
int ChunkManager::loadChunks(const string & str_dir_path)
{
    ...
    if ( FileSystemUtil::getAllFiles(vec_chunk_fullpath,str_dir_path,".chk") < 0){
        LOG_ERROR("ChunkManager::loadChunks failed. dir path:"<< str_dir_path);
        return -1;
    }
    ...
    for (int i=0; i < vec_size; ++i){
        ...
        if (id < 0 || file_size <= 0){
            LOG_ERROR("ChunkManager::loadChunks chunk error, path: "<<str_full_path<<", size:"<<file_size);
            continue;
        }
    ...
    LOG_DEBUG("Chunks:"<<chunk_log_info);
    LOG_INFO("Status: "<< m_chunks.size() << " chunks, "<< (size_sum/(1024*1024)) << " M Byte");
...
int ChunkManager::getNewChunkOutStream(std::ofstream & outfile,int64_t chunk_id)
{
    ...
    outfile.open(tmp_path.c_str(),ios::binary|ios::app);
    if(!outfile){
        LOG_ERROR("ChunkManager::getNewChunkOutStream() open tmp file path failed:"<<tmp_path);        
        return -1;
    }
    
    //    cout<<"Generate tmp file path: "<<tmp_path<<endl;
    LOG_INFO("ChunkManager::getNewChunkOutStream() generate tmp file path:"<<tmp_path);            
...
int ChunkManager::getChunkInStream(ifstream & infile,int64_t chunk_id)
{
    ...
    map<int64_t,ChunkInfo>::iterator it = m_chunks.find(chunk_id);
    if(it==m_chunks.end()){
        LOG_ERROR("ChunkManager::getChunkInStream() can't find chunk id:"<<chunk_id);
        return -1;
    }

    ChunkInfo info=it->second;
    
    infile.open((info.mPath).c_str(), ios::binary);
    
    if(!infile){
        LOG_ERROR("ChunkManager::getChunkInStream() can't open file path:"<<info.mPath);
        //@todo 处理出错
        return -1;
    }
    LOG_DEBUG("ChunkManager::getChunkInStream() open file for read:"<<info.mPath);    
...
//     string dest_path(ts("%s/%lld.chk",chunk_dir.c_str(),chunk_id));
    ...
    if (!FileSystemUtil::renameFile(chunk_info.mPath,dest_path)){
        LOG_ERROR("ChunkManager::commitChunk() rename file error!"<<chunk_info.mPath<<" -> "<<dest_path);
        //        cout<<"rename file error:"<<chunk_info.mPath<<" to: "<<dest_path<<endl;
        return -1;
    }
    

    LOG_INFO("ChunkManager::commitChunk() move"<<chunk_info.mPath<<" -> "<<dest_path);
