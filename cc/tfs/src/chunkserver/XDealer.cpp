...
XDealer::~XDealer()
{
    LOG_DEBUG(  "{"<<getName()<<"} ~XDealer() thread finish");        
}
...
void XDealer::operator()()
{
    while(mShouldContinue)
    {
        ...
        if (ret == 0){ //recv socket fin            
            ...
            LOG_ERROR(  "XDealer() {"<<getName()<<"} socket error when resv request msg! errno:"<<errno<<" means:"<<strerror(errno));            
            break;
        }
        ...
        if (request_msg.mCMD == READ){
            ...
            //@todo process error
            LOG_DEBUG(  "XDealer() {"<<getName()<<"} recv Client Disconnect");
            break;
        }else if (request_msg.mCMD == REMOTE_OP){
            
            LOG_DEBUG(  "XDealer() {"<<getName()<<"} remote operation connect.");
        ...
        // 	LOG_DEBUG(  "XDealer() {"<<getName()<<"} recv data");
        ...
        // }
            ...
            LOG_DEBUG(  "XDealer() {"<<getName()<<"} get local path.");
    ...
    if (mSockfd){
        LOG_DEBUG(  "XDealer() {"<<getName()<<"} task finish! running time:"<<getRunningTime()<<" seconds");
...
int XDealer::dealRead(const Client2ServerMsg& request_msg)
{
    ...
    //    LOG_DEBUG(  "Enter XDealer::dealRead()");
    TaskMgr * p_task_mgr = TaskMgr::getInstance();
    p_task_mgr->setDealerStatus(getName(),Read_Dealer);
    LOG_INFO( "XDealer::dealRead {"<<getName()<<"} read chunk id:"<<request_msg.mChunkID);
    ...
    if(ret<0){
        ...
        LOG_ERROR( "XDealer::dealRead {"<<getName()<<"} getChunkInStream error, chunk_id:"<<request_msg.mChunkID<<" ret:"<<ret);
    ...
    if(TCPUtil::SendData(mSockfd,(const char *)&return_msg,sizeof(return_msg))<0){
        ...
        LOG_ERROR( "in XDealer::dealRead():1 {"<<getName()<<"} send back msg error!");        
        return -1;
    }
    ...
    try{        
        ...
        while(send_size<request_msg.mDataLen){
            ...
            if(!chunk_reader){
                LOG_ERROR( "in XDealer::dealRead():2 {"<<getName()<<"} read data from file error");
...
//             LOG_DEBUG( "in XDealer::dealRead() {"<<getName()<<"} size of SockDataType:"
...
//             if(ret<=0){
//                 LOG_ERROR( "in XDealer::dealRead():3, send back data error, ret:"<<ret<<" sendsize:"<<send_size);        
//                 return -1;
//             }
        
            send_size+=read_size;
        }
        LOG_DEBUG( "in XDealer::dealRead() {"<<getName()<<"} read complete. size:"<<send_size);
        return send_size;
    }catch(TCPException & e){
        LOG_ERROR( "in XDealer::dealRead() {"<<getName()<<"} "<<e.what());        
        return -1;
    }
...
int XDealer::dealAppend(const Client2ServerMsg& request_msg)
{
    ...
    //    LOG_DEBUG(  "Enter XDealer::dealAppend() chunk id:"<<request_msg.mChunkID);
    LOG_INFO( "XDealer::dealAppend {"<<getName()<<"} append chunk id:"<<request_msg.mChunkID);
    ...
    if(ret < 0){ //错误, 告知client
        LOG_ERROR( "XDealer::dealAppend() {"<<getName()<<"} getChunkInStream error, chunk_id:"<<request_msg.mChunkID<<"ret:"<<ret);
    ...
    Server2ClientMsg return_msg(OK,OTHER);
    if(TCPUtil::SendData(mSockfd,(const char *)&return_msg,sizeof(return_msg))<0){
        LOG_ERROR( "XDealer::dealAppend():1 {"<<getName()<<"}  send back msg error");
        return -1;
    }
    ...
    while(i<pipe_line_num){
        ...
        if(ret<=0){
            LOG_ERROR( "XDealer::dealAppend():2 {"<<getName()<<"}  recv pipeline server addr error! ret: "<<ret);
        ...
        LOG_DEBUG( "XDealer::dealAppend() {"<<getName()<<"}  pipeline No."<<i<<"-->"<<server_addr.getIP()<<":"<<server_addr.getPort());
    }
    ...
    if(should_pipeline){
        if(chunk_transfer.initConnect(vec_servers)<0){
            LOG_ERROR( "XDealer::dealAppend():3 {"<<getName()<<"}  init pipeline error");
            should_pipeline=false;
        }else{
            if(chunk_transfer.addChunk(request_msg.mChunkID) < 0){
                LOG_ERROR( "XDealer::dealAppend():4 {"<<getName()<<"}  pipeline add chunk error");
                should_pipeline=false;
            }
        }
    }
    ...
    try{
            ...
            while (recv_size < this_time_data_len){                
                ...
                if(should_pipeline){
                    if(chunk_transfer.appendData(buf,should_read_size)<0){
                        should_pipeline=false;
                        LOG_ERROR( "XDealer::dealAppend():6 {"<<getName()<<"}  pipeline append data error, break pipeline");
        ...
        if (should_pipeline){
            if (chunk_transfer.finishAppend() < 0){
                should_pipeline = false;
                LOG_ERROR( "XDealer::dealAppend():6 {"<<getName()<<"}  pipeline finish error, break pipeline");
            }
        }
        
    }catch(TCPException &e){
        LOG_ERROR( "XDealer::dealAppend() {"<<getName()<<"} "<<e.what()<<" when append data len:"<<sum_size);
        return -1;
    }
...
//         if(ret<0){
//             //@todo 接受数据中出错,
//             LOG_ERROR( "XDealer::dealAppend():5, recvdata error");
...
//                 should_pipeline=false;
//                 LOG_ERROR( "XDealer::dealAppend():6, pipeline append data error, break pipeline");
//                 //                cout<<"dealAppend() pipeline break;"<<endl;
//             }
//         }
...
//     }
    ...
    if(pChunkManager->commitChunk(request_msg.mChunkID)<0){
        LOG_ERROR( "XDealer::dealAppend():7 {"<<getName()<<"} commit chunk error");
        //        cout<<"commit chunk error"<<endl;
    }
    LOG_DEBUG( "XDealer::dealAppend() {"<<getName()<<"} finish recv a new chunk, id:"<< request_msg.mChunkID <<" data len:"<<sum_size);    
    return 1;
}
...
{
    LOG_DEBUG( "XDealer::dealGetLocalPath() {"<<getName()<<"} get chunkid path: "<<request_msg.mChunkID);
    try{
        string local_path;
        if (pChunkManager->getChunkFilePath(local_path,request_msg.mChunkID) < 0){
            ...
            LOG_ERROR( "XDealer::dealGetLocalPath() {"<<getName()<<"} chunk don't exsit");
            return 1;
        }
        assert(local_path != "");
        ...
        LOG_ERROR( "XDealer::dealGetLocalPath() {"<<getName()<<"} "<<e.what());
        return -1;
    }
    return 1;
}
...
// 		if (data_len < 0){
// 			LOG_ERROR( "XDealer::dealPushData() data len have some problem");			    
// 			return -1;
// 		}
...
//                 if(chunk_transfer.appendData(buf,should_read_size)<0){
//                     should_pipeline=false;
//                     LOG_ERROR( "XDealer::dealPushData() {"<<getName()<<"}  pipeline append data error, break pipeline");
//                     //                cout<<"dealAppend() pipeline break;"<<endl;
//                 }
//             }
...
//         }
// 	}catch(TCPException &e){
//         LOG_ERROR( "XDealer::dealPushData() {"<<getName()<<"} "<<e.what());
//         return -1;
//     }
// 	return 1;
// }
