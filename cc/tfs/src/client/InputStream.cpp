...
int64_t InputStream::read(char *buf, int64_t nbytes) {
    LOG_INFO("InputStream: Try to read " << nbytes <<" bytes");
    if (!m_opened) {
    ...
    if (m_available >= nbytes){
        ...
        m_offset += nbytes;
        nread = nbytes;
        LOG_INFO("InputStream: Read " << nbytes <<" bytes from buffer");
    } else {
        ...
        if (m_available > 0) {
            ...
            nread = m_available;
            LOG_INFO("InputStream: Read " << nread <<" bytes from buffer");
        }
        if (m_bufferSize > nbytes) {
            ...
            m_file->seek(m_offset, SEEK_SET);
            int64_t nr = m_file->read(buf + nread, nbytes);
            nread += nr;
            m_offset += nr;
            m_used = 0;
            m_available = 0;
    ...
    LOG_INFO("InputStream: Read " << nread <<" bytes, current offset = " <<m_offset);
    return nread;	
}
