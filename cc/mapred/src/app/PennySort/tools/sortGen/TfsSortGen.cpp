...
// 20080503, port to linux, yzf, generate data into TFS
...
int main (int argc, char * argv[])
{
    ...
    try{
        ...
        tfsConfig.masterIP = "127.0.0.1";
        tfsConfig.masterPort = "20000";
        tfs::api::TFSConfiguration::set(tfsConfig);
        // create file
        tfs::api::FileSystem fs;
        try{
            /************************************************************************
    /*                                                                      *
...
//         }
            if (!fs.existFile(OutFileName)){
                printf("Create file %s\n", OutFileName);
                fs.createFile(OutFileName, 1, 0);
            }
            // open output file
            tfs::api::AppendStream as;
            as.open(OutFileName, 1024);
...
//             error = 1; goto common_exit;
//             }
                as.append(Buffer, (BufferRecords * RecordSize));
    ...
    /************************************************************************/
            ...
            as.close();
            if (Buffer != NULL) free(Buffer);                       // free record buffer
//     if (error){                          // delete output file on error
//         //remove( OutFileName);           //
//     }

        }catch(tfs::api::TFSException& ex){
            fprintf(stderr, "? Catch TFSException: %s\n", ex.what());
            fs.removeFile(OutFileName);
