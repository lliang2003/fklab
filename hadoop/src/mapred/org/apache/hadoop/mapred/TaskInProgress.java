 ...
 * so we need a level of indirection above the running-id itself. <br>
 * A given TaskInProgress contains multiple taskids, 0 or more of which might be
 * executing at any one time. (That's what allows speculative execution.) A
 * taskid is now *never* recycled. A TIP allocates enough taskids to account for
 * all the speculation and failures it will ever have to handle. Once those are
 * up, the TIP is dead.
 * **************************************************************
 */
class TaskInProgress {
   ...
   */
  public TaskInProgress(JobID jobid, String jobFile, RawSplit rawSplit,
      JobTracker jobtracker, JobConf conf, JobInProgress job, int partition) {
   ...
   */
  public TaskInProgress(JobID jobid, String jobFile, int numMaps, int partition,
      JobTracker jobtracker, JobConf conf, JobInProgress job) {
    this.jobFile = jobFile;
  ...
  public boolean isOnlyCommitPending() {
    for (TaskStatus t : taskStatuses.values()) {
      if (t.getRunState() == TaskStatus.State.COMMIT_PENDING) { return true; }
    }
    return false;
  }
  ...
  /**
   * Returns whether a component task-thread should be closed because the
   * containing JobInProgress has completed or the task is killed by the user
   */
  public boolean shouldClose(TaskAttemptID taskid) {
     ...
     * 
     * However, for completed map tasks we do not close the task which actually
     * was the one responsible for _completing_ the TaskInProgress.
     */
    boolean close = false;
    TaskStatus ts = taskStatuses.get(taskid);
    if ((ts != null)
        && (!tasksReportedClosed.contains(taskid))
        && ((this.failed) || ((job.getStatus().getRunState() != JobStatus.RUNNING && (job
            .getStatus().getRunState() != JobStatus.PREP))))) {
  ...
  /**
   * Creates a "status report" for this task. Includes the task ID and overall
   * status, plus reports for all the component task-threads that have ever been
   * started.
   */
  synchronized TaskReport generateSingleReport() {
    ...
    TaskReport report = new TaskReport(getTIPId(), (float) progress, state, diagnostics
        .toArray(new String[diagnostics.size()]), currentStatus, execStartTime,
        execFinishTime, counters);
   ...
   *          id of the task
   * @param diagInfo
   *          diagnostic information for the task
   */
  ...
  synchronized boolean updateStatus(TaskStatus status) {
    ...
    if (oldStatus != null) {
      ...
      if ((newState != TaskStatus.State.RUNNING
          ...
          && newState != TaskStatus.State.KILLED_UNCLEAN && newState != TaskStatus.State.UNASSIGNED)
          && (oldState == newState)) {
        LOG.warn("Recieved duplicate status update of '" + newState + "' for '" + taskid
            + "' of TIP '" + getTIPId() + "'");
        return false;
      }
      ...
      if ((newState == TaskStatus.State.RUNNING || newState == TaskStatus.State.UNASSIGNED)
          && (oldState == TaskStatus.State.FAILED || oldState == TaskStatus.State.KILLED
              || oldState == TaskStatus.State.FAILED_UNCLEAN
              || oldState == TaskStatus.State.KILLED_UNCLEAN
              || oldState == TaskStatus.State.SUCCEEDED || oldState == TaskStatus.State.COMMIT_PENDING)) { return false; }
    ...
    if (!isCleanupAttempt(taskid)) {
      ...
      taskStatuses.get(taskid).statusUpdate(status.getRunState(), status.getProgress(),
          status.getStateString(), status.getPhase(), status.getFinishTime());
    }
  ...
  public void incompleteSubTask(TaskAttemptID taskid, JobStatus jobStatus) {
    ...
    if (status != null) {
      ...
      if (taskState != TaskStatus.State.FAILED && taskState != TaskStatus.State.KILLED
        ...
        LOG.info("Task '" + taskid + "' running on '" + trackerName + "' in state: '"
            + taskState + "' being failed!");
  ...
  private boolean startSkipping() {
    if (maxSkipRecords > 0
        && numTaskFailures >= SkipBadRecords.getAttemptsToStartSkipping(conf)) { return true; }
    return false;
  }
   ...
   * 
   * @param taskId
   *          id of the completed task-attempt
   * @param finalTaskState
   *          final {@link TaskStatus.State} of the task-attempt
   */
  ...
  /**
   * Indicate that one of the taskids in this already-completed TaskInProgress
   * has successfully completed; hence we mark this taskid as
   * {@link TaskStatus.State.KILLED}.
   */
  ...
  /**
   * Indicate that one of the taskids in this TaskInProgress has successfully
   * completed!
   */
  ...
  public String[] getSplitLocations() {
    if (isMapTask() && !jobSetup && !jobCleanup) { return rawSplit.getLocations(); }
    return new String[0];
  }
  ...
  boolean killTask(TaskAttemptID taskId, boolean shouldFail) {
    ...
    if (st != null
        && (st.getRunState() == TaskStatus.State.RUNNING
            ...
            || st.inTaskCleanupPhase() || st.getRunState() == TaskStatus.State.UNASSIGNED)
        && tasksToKill.put(taskId, shouldFail) == null) {
      String logStr = "Request received to " + (shouldFail ? "fail" : "kill") + " task '"
          + taskId + "' by user";
  ...
  /**
   * This method is called whenever there's a status change for one of the TIP's
   * sub-tasks. It recomputes the overall progress for the TIP. We examine all
   * sub-tasks and find the one that's most advanced (and non-failed).
   */
  ...
  /**
   * Return whether the TIP has a speculative task to run. We only launch a
  ...
  public Task getTaskToRun(String taskTracker) throws IOException {
    ...
    if (nextTaskId < (MAX_TASK_EXECS + maxTaskAttempts + numKilledTasks)) {
      ...
      LOG.warn("Exceeded limit of " + (MAX_TASK_EXECS + maxTaskAttempts) + " (plus "
          + numKilledTasks + " killed)" + " attempts for the tip '" + getTIPId() + "'");
      return null;
    }
  ...
  public String getSplitNodes() {
    if (!isMapTask() || jobSetup || jobCleanup) { return ""; }
  ...
  /**
   * This class keeps the records to be skipped during further executions based
   * on failed records from all the previous attempts. It also narrow down the
   * skip records if it is more than the acceptable value by dividing the failed
   * range into half. In this case one half is executed in the next attempt
   * (test attempt). In the test attempt, only the test range gets executed,
   * others get skipped. Based on the success/failure of the test attempt, the
   * range is divided further.
   */
  private class FailedRanges {
    ...
    synchronized SortedRanges getSkipRanges() {
      if (divide != null) { return divide.skipRange; }
      return skipRanges;
    }
    ...
    synchronized long getIndicesCount() {
      if (isTestAttempt()) { return divide.skipRange.getIndicesCount(); }
      return skipRanges.getIndicesCount();
    }

    synchronized void updateState(TaskStatus status) {
      if (isTestAttempt() && (status.getRunState() == TaskStatus.State.SUCCEEDED)) {
    ...
    synchronized void add(Range failedRange) {
      ...
      if (divide != null) {
        LOG.warn("FailedRange:" + failedRange + "  test:" + divide.test + "  pass:"
            + divide.testPassed);
  ...
  public RawSplit[] getSplitParts(int numParts) throws IOException {
    ...
    for (int i = 0; i < numParts; ++i) {
      ...
      buffer.reset();
      serializer.serialize(psplit);
      rsplit.setDataLength(psplit.getLength());
      rsplit.setBytes(buffer.getData(), 0, buffer.getLength());
      rsplit.setLocations(psplit.getLocations());
      rsplits[i] = rsplit;
    }
    return rsplits;
  }
